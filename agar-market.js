...
export async function fetchAgarMarketPrice({
    ...
    if (!response.ok) throw new Error(`AGAR market provider returned ${response.status}`);
    ...
    if (!best) throw new Error('No AGAR/USD market pair is available');
    if (!Number.isFinite(best.liquidityUsd) || best.liquidityUsd < minLiquidityUsd) {
        throw new Error(`AGAR liquidity must be at least $${minLiquidityUsd}`);
    }
    cache = {
        mint,
